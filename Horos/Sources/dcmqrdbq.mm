...
//    } else {
//        CERR << "DB_DuplicateElement: out of memory" << std::endl;
//    }
//    }
//}
...
//	findRequestIdentifiers->print(COUT);
    ...
    if (!qrLevelFound) {
        ...
        CERR << "DB_startFindRequest(): missing Query/Retrieve Level" << std::endl;
        //handle->idxCounter = -1 ;
...
{
    return nextFindResponse(findResponseIdentifiers, status);
}
...
OFCondition DcmQueryRetrieveOsiriXDatabaseHandle::nextMoveResponse(
      ...
      DcmQueryRetrieveDatabaseStatus *status)
{
    return nextMoveResponse(SOPClassUID, SOPInstanceUID, imageFileName, numberOfRemainingSubOperations, status);
}
...
{
    return makeNewStoreFileName(SOPClassUID, SOPInstanceUID, newImageFileName);
}
...
OFCondition DcmQueryRetrieveOsiriXDatabaseHandle::storeRequest(
	...
	if( imageFileName == NULL || strcmp( imageFileName, NULL_DEVICE_NAME) == 0)
		return EC_Normal;
	...
	{
		NSLog( @"DcmQueryRetrieveOsiriXDatabaseHandle::storeRequest failed to move received file %@ to %@", sourcePath, destinationPath);
		if( status)
