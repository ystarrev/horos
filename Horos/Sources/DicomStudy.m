    ...
    if( dateOfBirth && acquisitionDate)
    {
        ...
        if( years < 2)
        {
            if( years < 1)
            {
                ...
                {
                    if( days < 0) return @"";
                    else return [NSString stringWithFormat: NSLocalizedString( @"%ld d", @"d = day"), (long)days];
                }
    ...
    if ([[self primitiveValueForKey:@"numberOfImages"] intValue] <= 0) // There are frames !
    {
        ...
        N2PerformManagedObjectContextBlockAndWait(self.managedObjectContext, ^{
            @try {
                ...
                result = [[NSNumber numberWithInt:sum] retain];
            }
            @catch (NSException* e) {
                N2LogExceptionWithStackTrace(e);
            }
...
- (NSNumber *) noFiles
{
    ...
    if (n == 0)
    {
        ...
        N2PerformManagedObjectContextBlockAndWait(self.managedObjectContext, ^{
            ...
            no = [[NSNumber numberWithInt: sum] retain];
...
+ (BOOL) displaySeriesWithSOPClassUID: (NSString*) uid andSeriesDescription: (NSString*) description containingOnlyPixels: (BOOL) pixels
{
    ...
    else
    {
        ...
        {
            if ([description hasPrefix: @"OsiriX ROI SR"])
                return YES;

            if ([description hasPrefix: @"OsiriX Annotations SR"] == NO && [description hasPrefix: @"OsiriX Report SR"] == NO && [description hasPrefix: @"OsiriX WindowsState SR"] == NO)
                return YES;
        }
...
- (NSManagedObject *) reportSRSeries
{
    ...
    N2PerformManagedObjectContextBlockAndWait(self.managedObjectContext, ^{
        ...
        if( [newArray count] > 1)
        {
            ...
            result = [[newArray lastObject] retain];
        }
        @catch (NSException* e) {
            N2LogExceptionWithStackTrace(e);
        }
...
- (DicomImage*) roiForImage: (DicomImage*) image inArray: (NSArray*) roisArray
{
    ...
    N2PerformManagedObjectContextBlockAndWait(self.managedObjectContext, ^{
        ...
        if( [found count] > 1)
        {
            ...
            for( DicomImage *i in found)
            {
                if( i != [found lastObject])
                {
                    @try
                    {
                        ...
                        if( d)
                        {
                            NSArray *o = [SRAnnotation unarchiveROIsFromCompatibilityData:d];
            ...
            if( [r count])
            {
                NSArray *o = [SRAnnotation unarchiveROIsFromCompatibilityData:[SRAnnotation roiFromDICOM:[[found lastObject] valueForKey:@"completePathResolved"]]];
                [r addObjectsFromArray: o];
        ...
        if(![[DicomDatabase databaseForContext:self.managedObjectContext] isLocal])
        {
            ...
            result = [[found lastObject] retain];
        }
        @catch (NSException* e) {
            N2LogExceptionWithStackTrace(e);
        }
...
- (NSString*) albumsNames
{
    ...
    N2PerformManagedObjectContextBlockAndWait(self.managedObjectContext, ^{
        @try {
            result = [[[[self.albums allObjects] valueForKey:@"name"] componentsJoinedByString:@"/"] retain];
        }
        @catch (NSException* e) {
            N2LogExceptionWithStackTrace(e);
        }
...
-(NSArray*) roiImages
{
    ...
    {
        NSArray *robjs = [SRAnnotation unarchiveROIsFromCompatibilityData:[SRAnnotation roiFromDICOM:[roi completePath]]];
