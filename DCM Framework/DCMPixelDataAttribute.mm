...
static void DCMRegisterDCMTKJPEGCodecs()
{
...
static const char *DCMUTF8StringOrDefault(NSString *value, const char *fallback)
{
    if (value.length == 0)
        return fallback;
...
static NSData *DCMDecodeEncapsulatedJPEGWithDCMTK(NSData *jpegData,
                                                  ...
                                                  int planarConfiguration)
{
    ...
    dataset.putAndInsertString(DCM_PhotometricInterpretation,
                               DCMUTF8StringOrDefault(photometricInterpretation, samplesPerPixel > 1 ? "RGB" : "MONOCHROME2"));
    if (samplesPerPixel > 1)
        dataset.putAndInsertUint16(DCM_PlanarConfiguration, static_cast<Uint16>(planarConfiguration > 0 ? planarConfiguration : 0));
    ...
    OFCondition status = fragment->putUint8Array(static_cast<const Uint8 *>(jpegData.bytes), static_cast<unsigned long>(jpegData.length));
    if (status.bad()) {
        delete fragment;
        delete offsetTable;
    ...
    OFString colorModel;
    status = datasetPixelData->getUncompressedFrame(&dataset, 0, startFragment, static_cast<Uint8 *>(rawData.mutableBytes), bufferSize, colorModel, nullptr);
    if (status.bad())
        return nil;

    if (bufferSize != frameSize)
        [rawData setLength:frameSize];
...
{
    NSString *photometricInterpretation = [_dcmObject attributeValueWithName:@"PhotometricInterpretation"];
    int planarConfiguration = [[_dcmObject attributeValueWithName:@"PlanarConfiguration"] intValue];
    return DCMDecodeEncapsulatedJPEGWithDCMTK(jpegData,
